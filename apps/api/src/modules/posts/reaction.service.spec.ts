...
describe('ReactionService', () => {
  ...
  test('toggle creates then deletes; counter goes 0 → 1 → 0', async () => {
    ...
    const first = await reactions.toggleLike(asUser(ctx.uuids.user.joon), 'POST', post.id);
    expect(first.liked).toBe(true);
    expect(first.like_count).toBe(1);

    const second = await reactions.toggleLike(asUser(ctx.uuids.user.joon), 'POST', post.id);
  ...
  test('two distinct users each contribute one like', async () => {
    ...
    const a = await reactions.toggleLike(asUser(ctx.uuids.user.joon), 'POST', post.id);
    expect(a.like_count).toBe(1);

    const b = await reactions.toggleLike(asUser(ctx.uuids.user.haneul), 'POST', post.id);
    expect(b.like_count).toBe(2);

    const c = await reactions.toggleLike(asUser(ctx.uuids.user.joon), 'POST', post.id);
