...
7. **[EVENTS_INTEGRATION.md](EVENTS_INTEGRATION.md)** — upstream events
   contract + failure matrix. Read if you are flipping
   `EVENTS_CLIENT_MODE=prism`. ~10 min.
8. **[ANALYTICS.md](ANALYTICS.md)** — first-party event taxonomy and
   privacy rules. Read if you are touching the analytics surfaces. ~10 min.
9. **[NEXT_BACKLOG.md](NEXT_BACKLOG.md)** — what is intentionally still
   deferred. Read to understand what you should NOT promise. ~10 min.
10. **[ALPHA_RC_CHECKLIST.md](ALPHA_RC_CHECKLIST.md)** — historical Alpha
